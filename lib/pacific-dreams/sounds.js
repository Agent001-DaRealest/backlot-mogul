...
// to activate the AudioContext. We register one-shot listeners on
// document so the very first tap/click/key creates and resumes the
// context long before any game sound is needed.
// ═══════════════════════════════════════════

const WARM_EVENTS = ['touchstart', 'touchend', 'mousedown', 'click', 'keydown'];

function removeWarmListeners() {
  WARM_EVENTS.forEach(evt =>
    document.removeEventListener(evt, warmAudio, { capture: true })
  );
}
...
export function warmAudio() {
  ...
  ctx.resume();
  // Play a 1-sample silent buffer to fully unlock the iOS audio session.
  // iOS Safari requires an actual AudioBufferSourceNode.start() during
  // a gesture — resume() alone is not enough.
...
function addWarmListeners() {
  WARM_EVENTS.forEach(evt =>
    document.addEventListener(evt, warmAudio, { once: true, passive: true, capture: true })
  );
}

// Self-register when module is first imported (SSR-safe)
...
function getCtx() {
  if (!ctx) {
    // Fallback: if somehow called before any gesture warmed the context
    ctx = new (window.AudioContext || window.webkitAudioContext)();
  }
