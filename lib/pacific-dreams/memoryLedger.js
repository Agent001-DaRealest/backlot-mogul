...
export const TAGS = {
  ...
  // Funding
  FUNDING_SELF:          'funding_self',
  FUNDING_DISTRIBUTOR:   'funding_distributor',
  TREASURY_DIP:          'treasury_dip',
  TREASURY_DIP_PATTERN:  'treasury_dip_pattern',
  MARKETING_SPEND:       'marketing_spend',
  MARKETING_FROM_TREASURY:'marketing_from_treasury',
  DISTRIBUTOR_REJECTED:  'distributor_rejected',
  ALWAYS_SELF_FUNDED:    'always_self_funded',
...
// Count consecutive self-funded films
export function selfFundStreak(ledger) {
  const funding = ledger.filter(e => e.tag === TAGS.FUNDING_SELF || e.tag === TAGS.FUNDING_DISTRIBUTOR);
  let count = 0;
  for (let i = funding.length - 1; i >= 0; i--) {
    if (funding[i].tag === TAGS.FUNDING_SELF) count++;
    else break;
  }
  return count;
}

// Count total treasury dips across all films
export function totalTreasuryDips(ledger) {
  return countTag(ledger, TAGS.TREASURY_DIP);
}
