...
export function calcPeriod(today, stock) {
  ...
  if (stock.eventDate) {
    ...
    if (daysSinceEvent >= 0 && daysSinceEvent <= 3) {
      return { period: 'CRUSH', left: 3 - daysSinceEvent, color: COLORS.green };
    }
  }

  // Check crush from lastEarn
  if (daysSinceLast >= 0 && daysSinceLast <= 3) {
    return { period: 'CRUSH', left: 3 - daysSinceLast, color: COLORS.green };
  }
  ...
  if (daysSinceNext >= 0 && daysSinceNext <= 3) {
    return { period: 'CRUSH', left: 3 - daysSinceNext, color: COLORS.green };
  }
