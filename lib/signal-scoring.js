...
export function calcPeriod(today, stock) {
  ...
  if (stock.eventDate) {
    ...
    if (daysSinceEvent >= 0 && daysSinceEvent <= 5) {
      return { period: 'CRUSH', left: 5 - daysSinceEvent, color: '#ffaa00' };
    }
  }

  if (daysSinceLast >= 0 && daysSinceLast <= 5) {
    return { period: 'CRUSH', left: 5 - daysSinceLast, color: '#ffaa00' };
  }
...
export function calcSignalData(price, w52h, w52l, iv, period, drawdownMode = 'NORMAL') {
  ...
  let priceScore = 0;
  if (pctAboveLow <= 10) priceScore = 3;
  else if (pctAboveLow <= 20) priceScore = 2;
  else if (pctAboveLow <= 50) priceScore = 1;

  const nearHighPenalty = pctBelowHigh < 20 ? -1 : 0;
  const ivScore = 0;
